...
public class PanierService implements IPanierService{
	...
	private boolean verifIdArticleBase(int user, int idArticle, int quantite){
		...
		lPanier = hPanierRep.getListPaniers(user);
	...
	public Panier insertArticle(Account user, Article article, int quantite) {
		...
		boolean verifArticleBDD = verifIdArticleBase(user.getIdUser(), article.getIdArticle(), quantite);
		...
		if (!verifArticleBDD) {
			...
			Panier hPanier = new Panier(user, article, quantite);
			...
			Integer oldQuantite = hPanierRep.getByUserAndArticle(user.getIdUser(), article.getIdArticle()).getQuantite();

			int newQuantite = oldQuantite + quantite;

			//modification de la ligne dans la bdd
			hPanierAdded = updatePanier(user.getIdUser(), newQuantite, article.getIdArticle());
	...
	public List<Panier> getListArticle(int id) {
		...
		List<Panier> hArticlePanier = hPanierRep.getListPaniers(id);
	...
	public List<Panier> addCadie(List<Panier> lPanier, Account user, Article article, int quantite){
		...
		if (!bIdTrouve) {
			...
			Panier newPanier = new Panier(user, article, quantite);
